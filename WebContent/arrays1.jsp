...
$(document).ready(function() {
	...
	$("[contenteditable=true]").on("keydown", function(e) {
		...
		if ($(this).text().length > max) {
			$('.introjs-tooltiptext').append("<div class='errorMsg'><br/>Enter a number of maximum 1 digits.</div>");
			e.preventDefault();
		}
	});
...
function introjsInit() {
	...
	introjs.onafterchange(function(targetElement) {
		...
		switch (elementId) {
			case "heading":
				$(".introjs-nextbutton").hide();
				var text = "Here we will learn how the memory is allocated for single and two dimensional array into the memory.";
			...
			case "twoDArrAnimation":
				$('.introjs-nextbutton').hide();
				$(".introjs-helperLayer").one("transitionend", function() {
					var text = "Click next to understand how these elements are stored continuously in memory.";
					typing(".introjs-tooltiptext", text, function() {
						$(".introjs-nextbutton").show();
					});
				});
...
function svgAnimation() {
	...
	if (count == parseInt($("#rows").text())) {
		...
		TweenMax.to($('#svgLine' + count + '1'), 1, {attr: {x2: x2, y2: y2}, onComplete: function() {
			...
			var x1 = $("#split" + count + "1").offset().left  - $(".merged-array").offset().left;
			var x2 = $("#split" + count + "1").offset().left + (($("#split" + count + "1").width()/2) - 10) - $(".merged-array").offset().left;
			var y1 = $("#split" + count + "1").offset().top - $(".merged-array").offset().top;
			var y2 = $("#split" + count + "1").offset().top - $(".merged-array").offset().top;
			...
			TweenMax.to($('#svgLine' + count + '2'), 1, {attr: {x2: x2, y2: y2}, onComplete: function() {
				...
				$("text").last().attr("x", (x - parseInt($("text").last().width() / 2) / ($("#svg").width() / 100)) + "%");
		...
		TweenMax.to($('#svgLine' + count + '3'), 1, {attr: {x2: x2, y2: y2}, onComplete: function() {
			...
			var x1 = $("#split" + count + "1").offset().left + ($("#split" + count + "1").width()) - $(".merged-array").offset().left - 4;
			var x2 = $("#split" + count + "1").offset().left + ($("#split" + count + "1").width() - ($("#split" + count + "1").width() / 2) - 10) - $(".merged-array").offset().left - 4;
			var y1 = $("#split" + count + "1").offset().top - $(".merged-array").offset().top;
			var y2 = $("#split" + count + "1").offset().top - $(".merged-array").offset().top;
...
function singleArrayAnimation() {
	...
	$("#singleArrInit").removeClass("opacity00").addClass("animated fadeIn").one("animationend", function() {
		...
		setTimeout(function() {
			...
			var text = "In a two-dimensional array we store the elements in <span class='ct-code-b-yellow'>rows</span>" 
