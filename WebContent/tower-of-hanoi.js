...
function initIntroJS() {
	...
	introjs.onafterchange(function(targetElement) {
		...
		if (introjs._currentStep > 1) {
			console.log(introjs._currentStep);
			introjs._introItems[introjs._currentStep - 1].currentAnimations = $("body").html();
		}
			    		...
			    		if (number >= 0) {
			     			$(fromId).effect( "transfer", { to: $("#invisibleDiv .panel"), className: "ui-effects-transfer" }, 1000, function() {
			 					$("#invisibleDiv .panel").removeClass("opacity00").addClass("animated fadeIn").one("animationend", function() {
									$(this).removeClass("animated zoomIn");
									//$("#stackBody .panel:eq(0) .print-stmt").css("word-spacing", "-3px");
									if (introjs._currentStep == 4) {
...
function createMainStack() {
	$("#stackBody").prepend("<div class='panel panel-primary opacity00'>" 
			...
			+ "<div><div id='mainStackNumDec1' style='display: inline-block;'>int disks = <div class='new-number' contenteditable='true' maxlength='0' style='display: inline-block;'>3</div>;</div></div>" 
